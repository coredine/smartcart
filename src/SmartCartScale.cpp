...
float calibrationKnownMass = 907.185; //2lb/907grams microphone, 200g/0.44lb phone
...
void calibrate() {
  LoadCells.tare();
  Serial.println("You have 15 seconds to place the known mass...");
  delay(15000);
  Serial.println("The 15 seconds ended...");
  LoadCells.refreshDataSet();
  calibrationFactor = LoadCells.getNewCalibration(calibrationKnownMass); //returns and sets
